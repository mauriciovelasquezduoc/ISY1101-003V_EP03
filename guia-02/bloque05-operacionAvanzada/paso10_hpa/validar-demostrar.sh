...
NAMESPACE="tienda"
BACKEND_SERVICE="tienda-backend"
...
echo "  Esperando pods de tienda-backend esten Ready..."
kubectl wait --for=condition=Ready pod -l app=tienda-backend -n $NAMESPACE --timeout=120s
echo "  ✔ Backend pods Ready."

echo ""
echo "  Verificando replicas iniciales..."
INITIAL_REPLICAS=$(kubectl get deployment tienda-backend -n $NAMESPACE -o jsonpath='{.status.availableReplicas}')
...
BACKEND_IMAGE=$(kubectl get deployment tienda-backend -n $NAMESPACE -o jsonpath='{.spec.template.spec.containers[0].image}' 2>/dev/null)
...
echo "  Monitoreando replicas de tienda-backend (max $MAX_WAIT_SCALE seg)..."
echo ""

ELAPSED=0
SCALED=false
while [ $ELAPSED -lt $MAX_WAIT_SCALE ]; do
  CURRENT_REPLICAS=$(kubectl get deployment tienda-backend -n $NAMESPACE -o jsonpath='{.status.availableReplicas}' 2>/dev/null || echo "0")
  HPA_STATUS=$(kubectl get hpa tienda-backend-hpa -n $NAMESPACE -o jsonpath='{..currentReplicas}' 2>/dev/null || echo "?")
...
if [ "$SCALED" = false ]; then
  ...
  kubectl describe hpa tienda-backend-hpa -n $NAMESPACE | grep -E "Metrics:|Reference:|Min replicas:|Max replicas:" || true
else
  echo ""
  echo "  Estado final del HPA:"
  kubectl get hpa tienda-backend-hpa -n $NAMESPACE
fi
