...
Este laboratorio despliega una aplicación de tienda online (frontend + backend + base de datos) en Amazon EKS usando infraestructura como código, CI/CD con GitHub Actions y observabilidad completa.
...
| Aspecto | Detalle |
|---------|---------|
| **¿Qué se hace?** | Crear tres repositorios privados en Amazon ECR: `tienda-db`, `tienda-backend`, `tienda-frontend` para almacenar las imágenes Docker. |
| **¿Qué se logra?** | Repositorios ECR listos para recibir imágenes. Tiempo estimado: ~2 min. |

```mermaid
flowchart LR
    A[Amazon ECR] --> B[tienda-db]
    A --> C[tienda-backend]
    A --> D[tienda-frontend]
...
| Aspecto | Detalle |
|---------|---------|
| **¿Qué se hace?** | Crea repositorios en GitHub, configura secrets (credenciales AWS), hace push del código fuente para que GitHub Actions construya y publique imágenes en ECR, y despliega los manifiestos Kubernetes en orden: MySQL → Backend API → Frontend Web con LoadBalancer. |
| **¿Qué se logra?** | Los 3 componentes (DB, Backend, Frontend) corriendo como Pods en el namespace `tienda`, con Services, HPA y Frontend expuesto mediante LoadBalancer. Tiempo estimado: ~15-20 min. |
...
| Aspecto | Detalle |
|---------|---------|
| **¿Qué se hace?** | Elimina todos los recursos en orden inverso: namespace `tienda`, stack CloudFormation del cluster EKS, stack CloudFormation de la VPC, repositorios ECR, repositorios GitHub, directorios locales clonados, entradas kubeconfig y known_hosts. |
| **¿Qué se logra?** | Laboratorio completamente limpio, listo para empezar desde cero. |

```mermaid
flowchart LR
    A[Limpiar] --> B[Namespace tienda<br/>pods + ELB]
