...
**Puerto:** 3001  
**Base de datos:** PostgreSQL (alumnos-db:3306)  
**Endpoints:**
...
| Archivo | Propósito |
|---|---|
| `k8s/namespace.yaml` | Namespace `alumnos` |
...
<!-- Copiar desde docs/reports/etapa09-ValidaApp.md o ejecutar: kubectl get hpa -n alumnos -->

```
$ kubectl get hpa -n alumnos
NAME                  REFERENCE          TARGETS       MINPODS MAXPODS REPLICAS
alumnos-backend-hpa    Deployment/backend cpu: 1%/70%   2       10      2
```
...
<!-- Copiar desde docs/reports/etapa09-ValidaApp.md o ejecutar: kubectl get pods -n alumnos -->

```
$ kubectl get pods -n alumnos -l app=alumnos-backend
NAME                              READY   STATUS    RESTARTS   AGE
alumnos-backend-6d7789595b-dcz55   1/1     Running   0          34m
alumnos-backend-6d7789595b-hdv6g   1/1     Running   0          34m
```

### Health check

```
$ curl -s http://alumnos-backend:3001/api/health
{"status":"ok","timestamp":"..."}
```
...
```
$ kubectl delete pod alumnos-backend-6d7789595b-dcz55 -n alumnos
pod "alumnos-backend-6d7789595b-dcz55" deleted

$ kubectl get pods -n alumnos -w
alumnos-backend-6d7789595b-xk9m2   Running   ← recreado en ~5s
```
...
```
$ kubectl logs deployment/alumnos-backend -n alumnos --tail=10
Server running on port 3001
Connected to PostgreSQL database: alumnos_data
