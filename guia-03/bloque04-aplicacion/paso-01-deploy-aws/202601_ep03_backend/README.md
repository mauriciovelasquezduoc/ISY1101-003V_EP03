...
### 1. Asegurarse que alumnos-db esta corriendo

Este compose asume que `alumnos-db` ya esta levantado. Si no lo esta:
...
```bash
docker ps --filter "name=alumnos-db"
```
...
```
NAME          STATUS       PORTS
alumnos-backend   Up X seconds 0.0.0.0:8080->8080/tcp
```
...
| Imagen ECR   | `alumnos-backend:latest`  |
| Puerto       | `8080`                |
...
```bash
docker build -t alumnos-backend:latest .
```
...
# Tag y push
docker tag alumnos-backend:latest <ECR_REGISTRY>/alumnos-backend:latest
docker push <ECR_REGISTRY>/alumnos-backend:latest
```
...
EC2-Web   (alumnos-frontend:latest)    — Capa Web    — Puerto 80
   |
EC2-App   (alumnos-backend:latest)    — Capa App    — Puerto 8080  <-- este servicio
   |
...
|---|---|---|
| Version semantica | `alumnos-backend:v1.4.0` | Rollback, trazabilidad |
| Latest | `alumnos-backend:latest` | Despliegue automatico |
...
- Hace pull de `alumnos-backend:latest` desde ECR via VPC Endpoint
