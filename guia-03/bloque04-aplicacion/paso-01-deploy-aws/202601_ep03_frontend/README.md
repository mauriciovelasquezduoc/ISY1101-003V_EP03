...
Este compose asume que `alumnos-backend` ya esta corriendo en la red `alumnos-network`.
...
```bash
cd ../alumnos-db && docker compose up -d
```

**3. Levantar el backend:**
```bash
cd ../alumnos-backend && docker compose up -d --build
```
...
```
NAME          STATUS       PORTS
alumnos-frontend   Up X seconds 0.0.0.0:80->80/tcp
```
...
Requiere que `alumnos-backend` este corriendo en `localhost:8080`.
...
| ------------- | ------------------ | ------------------------------ |
| Docker local  | `alumnos-backend`      | nombre del contenedor          |
...
| Imagen ECR   | `alumnos-frontend:latest`   |
| Puerto       | `80`                   |
...
```bash
docker build -t alumnos-frontend:latest .
```
...
# Tag y push
docker tag alumnos-frontend:latest <ECR_REGISTRY>/alumnos-frontend:latest
docker push <ECR_REGISTRY>/alumnos-frontend:latest
```
...
| -------------- | ------------------------------------ | ---------------- |
| `BACKEND_HOST` | Hostname o IP del backend (runtime)  | `alumnos-backend`    |
...
Internet
   |
EC2-Web   (alumnos-frontend:latest)    — Capa Web    — Puerto 80   <-- este servicio
   |  (proxy /alumnos → :8080)
EC2-App   (alumnos-backend:latest)    — Capa App    — Puerto 8080
   |
EC2-Datos (alumnos-db:latest)  — Capa Datos  — Puerto 5432
```
...
|---|---|---|
| Version semantica | `alumnos-frontend:v1.3.0` | Rollback, trazabilidad |
| Latest | `alumnos-frontend:latest` | Despliegue automatico |
...
**Pasos:**
1. Obtiene el Instance ID de `EC2-Web` desde **SSM Parameter Store** (`/alumnos/ec2/web`)
...
- Lee la IP privada de `EC2-App` desde SSM Parameter Store (`/alumnos/ec2/app/private-ip`)
- Hace pull de `alumnos-frontend:latest` desde ECR
